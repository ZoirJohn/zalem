		...
		<div className={cn("flex flex-col gap-6", className)} {...props}>
			<Card className="border-claude-hairline bg-claude-canvas text-claude-ink">
				<CardHeader className="text-center">
					...
					<CardDescription className="text-claude-body">Sign up with your Facebook or Google account</CardDescription>
				</CardHeader>
						...
						<FieldGroup className="gap-6">
								...
								children={(field) => {
									return (
										<Field>
											...
											<Input type="text" id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="Full name" autoComplete="name" aria-invalid={field.state.meta.isTouched && !field.state.meta.isValid} />
								...
								children={(field) => {
									return (
										<Field>
											...
											<Input type="email" id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="email@example.com" autoComplete="email" aria-invalid={field.state.meta.isTouched && !field.state.meta.isValid} />
											{field.state.meta.isTouched && !field.state.meta.isValid && <FieldError>{field.state.meta.errors?.[0]?.message}</FieldError>}
										</Field>
									);
								}}
									...
									children={(field) => {
										return (
											<Field>
												...
												<Input type="password" id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} autoComplete="new-password" aria-invalid={field.state.meta.isTouched && !field.state.meta.isValid} />
												{field.state.meta.isTouched && !field.state.meta.isValid && <FieldError>{field.state.meta.errors?.[0]?.message}</FieldError>}
											</Field>
										);
									}}
							...
							<form.Subscribe
								...
								children={() => {
									return (
										<Field>
											<Button type="submit" className="bg-claude-primary text-white hover:bg-claude-primary-active">
												Create Account
											</Button>
											<FieldDescription className="text-center text-claude-body">
												Already have an account?{" "}
												<Link className="text-claude-primary hover:text-claude-primary-active" to="/login">
													Sign in
												</Link>
											</FieldDescription>
